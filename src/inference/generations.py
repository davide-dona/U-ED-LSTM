...
from configs.event_log import CASE_ELAPSED_COLUMN, UNKNOWN_LABEL
...
class GenerationDecoder:
    ...
    def rows(self,
        ...
        - sampled: `(activities, lengths, elapsed)` as `SuffixSampler.sample` returned them.
        - point: `(activities, lengths, elapsed)` as `SuffixSampler.point` returned them.
        ...
        prefix_elapsed = self._minutes(
            dataset.continuous[starts + cut_points - 1, self.elapsed_channel])
        true_elapsed = self._minutes(
            dataset.continuous[starts + real_lengths - 1, self.elapsed_channel])

        sampled_activities, sampled_lengths, sampled_elapsed = (tensor.cpu().numpy()
                                                                for tensor in sampled)
        point_activities, point_lengths, point_elapsed = (tensor.cpu().numpy()
                                                          for tensor in point)
        sampled_remaining = self._remaining(self._minutes(sampled_elapsed),
                                            prefix_elapsed[:, None])
        point_remaining = self._remaining(self._minutes(point_elapsed), prefix_elapsed)
    ...
    def _minutes(self, encoded : np.ndarray) -> np.ndarray:
        """Encoded `case_elapsed_time` back in the log's own minutes, keeping the input's shape."""
        return self.elapsed.denormalize(encoded)
